...
@section('content')
    ...
    <div class="layui-fluid">
        ...
        <div class="op-container">
            <div class="left"></div>
            <div class="right">
                <button  class="layui-btn layui-btn-custom-blue" onclick="setPrice('set')">设置产品价格</button>
            </div>
        </div>
                ...
                <div class="layui-form-item" style="margin-top:20px;">
                    <div class="set-price-block">
                        <div class="layui-inline">
                            <div class="layui-input-inline" style="padding-right:0;margin-right:0;" >
                                <input type="radio" name="type" value="{{\App\Models\ProductCeramic::PRICE_WAY_UNIFIED}}" title="统一定价" checked/>
                            </div>
                            ...
                            <div class="layui-input-inline" style="width: 80px;padding-top:5px;">
                                <select name="unit" lay-verify="">
                                    @foreach(\App\Models\ProductCeramicAuthorization::unitGroup() as $key=>$item)
                                        <option value="{{$key}}"  @if($key==0) selected @endif>{{$item}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div  class="set-price-block">
                        <div class="layui-inline">
                            <div class="layui-input-inline" style="padding-right:0;margin-right:0;" >
                                <input type="radio" name="type" value="{{\App\Models\ProductCeramic::PRICE_WAY_FLOAT}}" title="浮动定价" />
                            </div>
                            <div class="layui-input-inline" style="width: 100px;padding-top:5px;">
                                <input type="text" name="price" placeholder="0.00" autocomplete="off" class="layui-input">
                            </div>
                            ...
                            <div class="layui-form-mid" style="padding-top: 11px!important;">上下浮动</div>
                            <div class="layui-input-inline" style="width: 80px;padding-top:5px;">
                                <input type="text" name="float" placeholder="0.00" autocomplete="off" class="layui-input">
                            </div>
                            <div class="layui-form-mid" style="padding-top: 11px!important;">%</div>
                        </div>
                    </div>
                    <div  class="set-price-block">
                        <div class="layui-inline">
                            <div class="layui-input-inline" style="padding-right:0;margin-right:0;" >
                                <input type="radio" name="type" value="{{\App\Models\ProductCeramic::PRICE_WAY_CHANNEL}}" title="渠道定价"/>
                            </div>
                        </div>
                    </div>
                    <div  class="set-price-block">
                        <div class="layui-inline">
                            <div class="layui-input-inline" style="padding-right:0;margin-right:0;" >
                                <input type="radio" name="type" value="{{\App\Models\ProductCeramic::PRICE_WAY_NOT_ALLOW}}" title="不定价"/>
                            </div>
                        </div>
                    </div>
...
@section('script')
    ...
    <script>
        ...
        let base_api = '{{url('/admin/brand/product/authorize/price/object/api')}}';
        let table_api = '{{url('/admin/brand/product/authorize/price/object/api')}}';
        //转换静态表格
        ...
        function setPrice(operation) {
            ...
            layer.open({
                ...
                yes:function(index, layero){
                    confirmSetPrice(index,layero);
                }
            });
        }

        function confirmSetPrice(layer_index,layero){
            //获取选择的定价方式
            var type_obj = layero.find("input[name='type']:checked");
            var price_type = type_obj.val();
            var set_price_block = type_obj.parents('.set-price-block');
            var price = set_price_block.find("input[name='price']")?set_price_block.find("input[name='price']").val():0;
            var unit = set_price_block.find("select[name='unit']")?set_price_block.find("select[name='unit']").val():0;
            var float = set_price_block.find("input[name='float']")?set_price_block.find("input[name='float']").val():0;
            //显示loading
            layer.load(1);
            //获取销售商选中行
            var checkStatus = table.checkStatus('tableInstance');
            ...
            ajax_post('{{url('admin/brand/product/authorize/price/object/api/set_price')}}',
                {
                    pids:product_ids,oids:object_ids,op:set_operation,
                    type:price_type,price:price,unit:unit,float:float
                }, function (res) {
                    if (res.status == 1) {
                        layer.msg('操作成功！');
                        layer.closeAll('loading');
