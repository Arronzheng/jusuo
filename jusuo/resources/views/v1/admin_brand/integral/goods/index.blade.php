...
@section('content')
    ...
    <script type="text/html" id="tableToolbarTpl">
        ...
        @{{#     if( d.is_import ){       }}
        <a href="javascript:;" title="取消引入" onclick="destroyData('@{{d.id}}','@{{d.deleteApiUrl}}','取消引入')" class="layui-btn-danger layui-btn  layui-btn-xs" lay-event="edit">
            <i class="layui-icon layui-icon-delete"></i>
        </a>
        @{{#     }else{     }}
        <a href="javascript:;" title="删除" onclick="destroyData('@{{d.id}}','@{{d.deleteApiUrl}}','删除')" class="layui-btn-danger layui-btn  layui-btn-xs" lay-event="edit">
            <i class="layui-icon layui-icon-delete"></i>
        </a>
        @{{#     }     }}
        @endcan
...
@section('script')
    ...
    <script>
        ...
        let base_api = '{{url('/admin/brand/integral/goods/api')}}';
        let table_api = '{{url('/admin/brand/integral/goods/api')}}';
        //转换静态表格
        ...
        let tableInstance = table.render({
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'is_import_text',width:110, title: '是否引入商品'}
                ...
                ,{field: 'brand_name',width:120, title: '品牌'}
                ,{field: 'category_text',width:120, title: '分类'}
    ...
    <script>
        ...
        function editData(type,id){
            let page = '{{url('/admin/brand/integral/goods/create')}}';
            if(type=='edit'){
        ...
        //删除数据
        function destroyData(id,deleteApi,opTypeText){
            let apiUrl = deleteApi;
            layer.confirm('确定'+opTypeText+'吗?', {icon: 3, title:'提示'}, function(index){
                ajax_post(apiUrl,{
                    '_method':'DELETE',
                    'id':id,
                },function(result){
                    if(result.status){
                        layer.msg(opTypeText+'成功！')
                        reloadTable()
                    }else{
        ...
        function export_table(){
            ...
            $('#filter-form').attr('action',"{{url('/admin/brand/integral/goods/api')}}");
            $('#filter-form').submit();
        }
