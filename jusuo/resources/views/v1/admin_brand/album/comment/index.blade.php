...
@section('content')
            ...
            <form action="" method="get">
                <div class="layui-form-item">
                    ...
                    <div class="layui-inline filter-block">
                        <label class="layui-form-label">方案标题/编号：</label>
                    ...
                    <div class="layui-inline filter-block">
                        ...
                        <div class="layui-input-inline">
                            <input type="text" name="reviewer" autocomplete="off" value="{{request()->input('reviewer')? : ''}}" placeholder="" class="layui-input">
                        </div>
                    </div>
                    <div class="layui-inline filter-block">
                        ...
                        <div class="layui-input-inline" style="padding-right: 14px">
                            <select name="status" lay-verify="">
                                ...
                                @foreach(\App\Models\AlbumComments::statusGroup() as $key=>$item)
    ...
    <script type="text/html" id="tableToolbarTpl">
        ...
        @can('album_manage.comment_switch')

        @{{#     if( d.status ){       }}
        <a href="javascript:;" title="屏蔽" onclick="ajax_status('@{{d.changeStatusApiUrl}}')" class="layui-btn-danger layui-btn  layui-btn-xs" lay-event="edit">
            <i class="layui-icon layui-icon-close"></i>
        </a>
        @{{#     }else{     }}
        <a href="javascript:;" title="显示" onclick="ajax_status('@{{d.changeStatusApiUrl}}')" class="layui-btn-success layui-btn  layui-btn-xs" lay-event="edit">
...
@section('script')
    ...
    <script>
        ...
        let base_api = '{{url('/admin/brand/album/comment/api')}}';
        let table_api = '{{url('/admin/brand/album/comment/api')}}';
        //转换静态表格
        ...
        let tableInstance = table.render({
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'album_title', title: '方案名称'}
                //,{field: 'album_code', title: '方案编码',width:150}
                ,{field: 'commenter_nickname', title: '评论者',width:100}
    ...
    <script>
        ...
        function editData(type,id){
            let page = '{{url('/admin/brand/album/comment/create')}}';
            if(type=='edit'){
                page = '{{url('/admin/brand/album/comment')}}/'+id+"/edit";
            }
            layer.open({
                type: 2,
                title:'编辑信息',
