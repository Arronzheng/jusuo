...
@section('content')
    <div class="layui-card layadmin-header">
        <div class="layui-breadcrumb" lay-filter="breadcrumb" style="visibility: visible;">
            ...
            <div class="right" style="margin-right:15px;">
                @can('album_manage.verify.hd_photo.brand_designer_album_index')
                <button onclick="location.href='{{url('/admin/brand/album/verify/hd/brand_designer')}}'" class="layui-btn layui-btn-sm layui-btn-custom-blue" >
                    <i class="layui-icon layui-icon-sm layui-icon-link" style="font-size:12px!important;"></i>方案审核
                </button>
                @endcan
    ...
    <script type="text/html" id="tableToolbarTpl">
        ...
        @can('album_manage.album_index.hd_photo.brand_designer_album_detail')
        {{--<a onclick="openDetail('@{{d.web_id_code}}')" title="查看详情" href="javascript:;"  class="layui-btn-custom-blue layui-btn  layui-btn-xs">
            <i class="layui-icon layui-icon-list"></i>
        </a>--}}
        <a title="预览" target="_blank" href="/admin/brand/album/index/common/preview_album_detail/@{{d.web_id_code}}"  class="layui-btn-custom-blue layui-btn  layui-btn-xs">
            <i class="layui-icon layui-icon-search"></i>
        </a>
        @endcan

        {{--品牌设计师方案的品牌展示上下架--}}
        @can('album_manage.album_index.hd_photo.brand_designer_album_switch')
        @{{#  if( d.designer_type_code == 2 ){  }}
            @{{#     if( d.status_brand>0 ){       }}
            <a href="javascript:;" title="品牌展示下架" onclick="ajax_status('@{{d.changeStatusApiUrl}}')" class="layui-btn-danger layui-btn  layui-btn-xs" lay-event="edit">
                <i class="layui-icon layui-icon-close"></i>
            </a>
            @{{#     }else{     }}
            <a href="javascript:;" title="品牌展示上架" onclick="ajax_status('@{{d.changeStatusApiUrl}}')" class="layui-btn-success layui-btn  layui-btn-xs" lay-event="edit">
        ...
        {{--销售商设计师方案的品牌展示上下架--}}
        @{{#  if( d.designer_type_code == 1 ){  }}
            @{{# if(d.status_brand == d.codeStatusBrandVerifying ){ }}
            {{--当销售商方案正在申请品牌展示时，可以上架/下架--}}
            @can('album_manage.verify.hd_photo.seller_album_apply_brand_pass')
            <a href="javascript:;" title="品牌展示上架" onclick="ajax_pass_album('@{{d.approveStatusApiUrl}}')" class="layui-btn-success layui-btn layui-btn-xs" lay-event="edit">
                <i class="layui-icon layui-icon-ok"></i>
            </a>
            @endcan
            @can('album_manage.verify.hd_photo.seller_album_apply_brand_pass')
            <a href="javascript:;" title="品牌展示驳回" onclick="ajax_reject_album('@{{d.rejectStatusApiUrl}}')" class="layui-btn-danger layui-btn layui-btn-xs" lay-event="del">
                <i class="layui-icon layui-icon-close"></i>
            </a>
            @endcan
            @{{# } }}
        ...
        @can('album_manage.verify.hd_photo.seller_album_apply_brand_reject')
        @{{#  if( d.designer_type_code == 1 ){  }}
            {{--当销售商方案已经在品牌展示时，可以操作下架--}}
            @{{# if (d.status_brand > d.codeStatusBrandVerifying){ }}
            <a href="javascript:;" title="品牌展示下架" onclick="ajax_reject_album('@{{d.rejectStatusApiUrl}}')" class="layui-btn-danger layui-btn layui-btn-xs" lay-event="del">
                <i class="layui-icon layui-icon-close"></i>
            </a>
            @{{# } }}
        @{{# } }}
        ...
        @can('album_manage.album_index.hd_photo.brand_designer_album_apply_platform')
        @{{#     if( d.can_switch ){       }}
...
@section('script')
    ...
    <script>
        ...
        $(function () {
            ...
            layui.use(['jquery','tableSelect'], function () {
                ...
                tableSelect.render({
                    ...
                    table: {
                        url:'{!! url($url_prefix.'admin/brand/album/index/common/api/get_brands') !!}',
                        cols: [[
                            { type: 'radio' },
        ...
        let base_api = '{{url('/admin/brand/album/index/common/api')}}';
        let table_api = '{{url('/admin/brand/album/index/common/api')}}';
        //转换静态表格
        ...
        let tableInstance = table.render({
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'status_text',width:90 ,fixed:'right', title: '审核状态'}
                ,{field: 'status_brand_text',width:80 ,fixed:'right', title: '展示状态'}
    ...
    <script>
        ...
        function ajax_pass_album(url) {
            layer.confirm('你确定审核通过该方案在品牌显示吗？', {icon: 3, title: '提示！'}, function () {
                //显示loading
                layer.load(1);
        ...
        function reloadTable(sortObj){
            table.reload('tableInstance',{
                ...
                ,where: {
                    ...
                    ,dsntype: $('select[name="dsntype"]').val()
