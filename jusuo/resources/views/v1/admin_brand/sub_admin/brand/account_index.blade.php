...
@section('content')
    <div class="layui-card layadmin-header">
        <div class="layui-breadcrumb" lay-filter="breadcrumb" style="visibility: visible;">
            ...
            <div class="right" style="margin-right:15px;">
                @can('account_manage.role_index')
                <button onclick="location.href='{{url('/admin/brand/role')}}'" class="layui-btn layui-btn-sm layui-btn-custom-blue" >
                    <i class="layui-icon layui-icon-sm layui-icon-link" style="font-size:12px!important;"></i>角色列表
                </button>
                @endcan
            </div>
        </div>
    ...
    @can('account_manage.account_index.brand_admin_create')
    ...
    <script type="text/html" id="tableToolbarTpl">
        ...
        @can('account_manage.account_index.brand_admin_detail')
        ...
        @can('account_manage.account_index.brand_admin_edit')
        @{{#    if( d.canEdit ){     }}
        @{{#    if(! d.is_super_admin ){     }}
        ...
        @can('account_manage.account_index.brand_admin_modify_pwd')
        @{{#    if( d.canModifyPwd ){     }}
        ...
        @can('account_manage.account_index.brand_admin_switch')
        @{{#    if( d.canEdit ){     }}
            @{{#    if(! d.is_super_admin ){     }}
                @{{#     if( d.isOn ){       }}
        ...
        @can('account_manage.account_index.brand_admin_delete')
        @{{#    if( d.canEdit ){     }}
            @{{#    if(! d.is_super_admin ){     }}
...
@section('script')
    ...
    <script>
        ...
        let base_api = '{{url('/admin/brand/sub_admin/api/account')}}';
        let table_api = '{{url('/admin/brand/sub_admin/api/account')}}?type={{request()->input("type")}}';
        //转换静态表格
        ...
        let tableInstance = table.render({
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'operation',fixed: 'right', title: '操作' , width:200 ,templet:'#tableToolbarTpl'}
    ...
    <script>
        ...
        function editData(type,id){
            let page = '{{url('/admin/brand/sub_admin/account/create')}}';
            if(type=='edit'){
                page = '{{url('/admin/brand/sub_admin/account')}}/'+id+"/edit";
            }
            layer.open({
                type: 2,
                title:'编辑信息',
        ...
        function modifyPwd(id){
            let page = '{{url('/admin/brand/sub_admin/account')}}/'+id+"/modify_pwd";
            layer.open({
                type: 2,
                title:'修改密码',
                area:['700px', '500px'],
        ...
        function openDetail(id){
            let page = '{{url('/admin/brand/sub_admin/account')}}/'+id;
            layer.open({
                type: 2,
                title:'查看详情',
                area:['700px', '500px'],
        ...
        function checkPrivilege(id){
            let page = '{{url('/admin/brand/sub_admin/account/privilege')}}/'+id;
            layer.open({
                type: 2,
                title:'管理员权限',
                area:['700px', '500px'],
