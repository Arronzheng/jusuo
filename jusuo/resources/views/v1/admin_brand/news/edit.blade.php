        ...
        <form>
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <input type="text" name="title" value="{{$data->title or ''}}" lay-verify="required" placeholder="" autocomplete="off" class="layui-input">
                </div>
            </div>
            <div class="layui-form-item layui-form-text">
                <label class="layui-form-label">图文资讯</label>
                <div class="layui-input-block" >
                    <textarea class="layui-textarea layui-hide" id="editor-instance">{!! $data->content or '' !!}</textarea>
                </div>
            </div>
            <div class="layui-form-item">
...
@section('script')
    ...
    <script>
        ...
        var uploadAvatar = upload.render({
            elem: '#b-upload-image'
            ,url: '{{url($url_prefix.'admin/brand/news/api/upload_image')}}'
    ...
    <script>
        ...
        let submitAction = "{{url('/admin/brand/news/api')}}";
        let submitMethod = "";
        @if(isset($data))
            submitAction = "{{url('/admin/brand/news/api')}}/{{$data->id}}";
            submitMethod = "PUT";
        @endif
        ...
        form.on('submit(submitFormBtn)', function(form_info){
            ...
            form_field.content = layedit.getContent(editor1);
            //提交方式
            if(submitMethod){
