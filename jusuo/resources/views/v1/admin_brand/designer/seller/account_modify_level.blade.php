            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <select name="level" lay-verify="">
                        @foreach(\App\Models\DesignerDetail::designerLevelIdNameGroup(true) as $item)
                            <option value="{{$item['id'] or ''}}"  @if($detail->self_designer_level== $item['id']) selected @endif>{{$item['name'] or ''}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
...
@section('script')
    ...
    <script>
        ...
        let submitAction = "{{url('/admin/brand/seller_designer/api/account')}}/{{$data->id}}/modify_level";
