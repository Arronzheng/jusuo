...
@section('content')
    ...
    <script type="text/html" id="tableToolbarTpl">
        ...
        @if(
        auth()->user()->can('account_manage.account_index.designer.brand_designer_detail') ||
        auth()->user()->can('account_manage.account_index.designer.seller_designer_detail')
        )
        @{{#    if( d.status=='200' ){     }}
        <a title="预览详情" target="_blank" href="/admin/brand/designer/preview_designer_detail/@{{d.web_id_code}}"  class="layui-btn-primary layui-btn  layui-btn-xs">
        ...
        @{{#    if( d.org_type=='brand' ){     }}
        <a onclick="modifyLevel('@{{d.id}}','@{{d.org_type}}')" title="修改等级" href="javascript:;"  class="layui-btn-custom-blue layui-btn  layui-btn-xs">
            <i class="layui-icon layui-icon-template-1"></i>
        </a>
        @{{#    }     }}
        @endcan

        @can('account_manage.account_index.designer.seller_designer_level')
        ...
        @{{#    if( d.org_type=='brand' || d.org_type=='seller' ){     }}
        <a onclick="modifyIntegral('@{{d.id}}','@{{d.org_type}}')" title="积分调整" href="javascript:;"  class="layui-btn-custom-blue layui-btn  layui-btn-xs">
...
@section('script')
    ...
    <script>
        ...
        let base_api = '{{url('/admin/brand/designer/api/account')}}';
        let table_api = '{{url('/admin/brand/designer/api/account')}}?type={{request()->input("type")}}';
        //转换静态表格
        ...
        let tableInstance = table.render({
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'self_birth_time',width:160,title: '出生年月日'}
                ...
                //,{field: 'point_experience',width:80,title: '经验值'}
                ,{field: 'status_text',width:120, title: '账号状态'}
                ,{field: 'created_at',width:160, title: '注册时间'}
                ,{field: 'approve_info',width:160, title: '认证时间'}
                ,{field: 'status_text',width:80, fixed:'right', title: '账号状态'}
                ,{field: 'operation',fixed: 'right', title: '操作' , width:210 ,templet:'#tableToolbarTpl'}
    ...
    <script>
        ...
        function show_integral_log(id){
            let page = '{{url('/admin/brand/designer/integral_log')}}/'+id;
            layer.open({
                type: 2,
                title:'积分变动明细',
                area:['900px', '500px'],
        ...
        function show_exchange_log(id){
            let page = '{{url('/admin/brand/integral/exchange_log')}}?w=1&d='+id;
            layer.open({
                type: 2,
                title:'兑换记录',
                area:['900px', '500px'],
        ...
        //打开修改等级框
        function modifyLevel(id,org_type){
            let page = '{{url('/admin/brand/brand_designer/account')}}/'+id+"/modify_level";
            if(org_type=='seller'){
                page = '{{url('/admin/brand/seller_designer/account')}}/'+id+"/modify_level";
            }
            layer.open({
                type: 2,
                title:'修改等级',
        ...
        //打开修改等级框
        function modifyIntegral(id,org_type){
            let page = '{{url('/admin/brand/brand_designer/account')}}/'+id+"/modify_integral";
            if(org_type=='seller'){
                page = '{{url('/admin/brand/seller_designer/account')}}/'+id+"/modify_integral";
            }
            layer.open({
                type: 2,
                title:'积分调整',
        ...
        function openDetail(id,org_type){
            ...
            if(org_type=='seller'){
                page = '{{url('/admin/brand/seller_designer/account')}}/'+id;
            }
            layer.open({
                type: 2,
                title:'查看详情',
