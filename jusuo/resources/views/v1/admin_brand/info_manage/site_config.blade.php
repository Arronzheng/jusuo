                ...
                <div class="layui-card-body" style="padding: 15px;">
                    <form class="layui-form" action="" lay-filter="component-form-group">
                        ...
                        <div class="layui-form-item">
                            ...
                            <div class="layui-input-inline">
                                <input type="text" name="front_name" value="{{$content['front_name'] or ''}}"   autocomplete="off" placeholder="" class="layui-input">
                            </div>
                        </div>
                        <div class="layui-form-item">
                            <label class="layui-form-label">管理后台标题</label>
                            <div class="layui-input-inline">
                                <input type="text" name="tool_name" value="{{$content['tool_name'] or ''}}"   autocomplete="off" placeholder="" class="layui-input">
                            </div>
                        </div>
                        <div class="layui-form-item">
                            <label class="layui-form-label">积分规则链接</label>
                            <div class="layui-input-inline">
                                <input type="text" name="integral_rule_link" value="{{$content['integral_rule_link'] or ''}}"   autocomplete="off" placeholder="" class="layui-input">
                            </div>
                        </div>
                        <div class="layui-form-item">
                            <label class="layui-form-label">管理后台登录背景图</label>
                            <div class="layui-input-inline">
                                <div class="layui-upload-drag" id="b-upload-admin-bg">
                                    <i class="layui-icon"></i>
                                    <p>仅支持JPG/PNG格式，每张图片大小限制2M以内</p>
                                    <input type="hidden" name="admin_login_bg" value="{{$content['admin_login_bg'] or ''}}"/>
                                    <div class="upload-img-preview" style="background-image:url('{{$content['admin_login_bg'] or ''}}')"></div>
                                </div>
                            </div>
                            <div class="layui-btn layui-btn-xs layui-btn-primary" onclick="delete_admin_bg()" style="display:inline-block">删除背景图</div>
                        ...
                        <div class="layui-form-item">
                            ...
                            <div class="layui-input-inline">
                                <button type="button" class="layui-btn" lay-submit lay-filter="submitFormBtn">立即保存</button>
...
@section('script')
    <script>
        ...
        //管理后台登录背景图片上传
        var uploadAdminLoginBg = upload.render({
            elem: '#b-upload-admin-bg'
            ,url: '{{url($url_prefix.'admin/brand/api/upload_avatar')}}'
            ,data: {'_token':'{{csrf_token()}}'}
            ,size:avatar_size  //KB
            ,acceptMime: 'image/jpeg,image/jpg,image/png'
            ...
            ,done: function(res){
                ...
                //上传成功
                $('#b-upload-admin-bg .upload-img-preview').css('background-image','url('+res.data.access_path+')');
                $('#b-upload-admin-bg input').val(res.data.access_path);
            }
        });
        ...
        form.on('submit(submitFormBtn)', function(form_info){
            ...
            ajax_post('{{url($url_prefix.'admin/brand/api/submit_site_config')}}',
                form_field,
                function(result){
                    // console.log(result);
                    if(result.status){
    ...
    <script>
        ...
        //删除后台登录背景图
        function delete_admin_bg(){
            $('#b-upload-admin-bg input').val('');
            $('#b-upload-admin-bg .upload-img-preview').attr('style','');
        }
