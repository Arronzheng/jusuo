...
@section('content')
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="left">
                ...
                <div class="product navigator-location" style="background-color:#ffffff;padding-top:30px;padding-bottom:40px;" >
                    ...
                    <div class="lookall" id="album_similiar_lookall" data-url="" onclick="click_more_similiar(this)">更多相似方案 ></div>
                </div>
                <div class="product navigator-location" style="background-color:#ffffff;padding-top:30px;padding-bottom:30px;" >
                    <div class="head">
                        <label class="producttitle">评论（<span class="comment-total">0</span>）</label>

                    </div>
                    <div id="fb_pinglun">
                        <div class="pinglunblock" placeholder="写下您的评论..." id="comment-input" contenteditable="true" onkeyup="check_comment_content()"></div>
                        <div style="display:none;" id="comment-input-shadow"></div>
                        <div class="button2" type="submit" onclick="commit_comment()" id="btnButton" style="background-color: rgb(217, 217, 217); cursor: not-allowed;">发表</div>
                    </div>
                    <div id="pinglun">
    ...
    <div id="qrcode" class="qrcode" style="display:none;">
        <div class="ftext">
            <span>分享到微信朋友圈</span>
            <a class="close" onclick="closefriend()">×</a>
        </div>
        <div class="wx-bd">
            <div class="qrimage"></div>
        </div>
        <div class="ftext" style="margin-top:-10px;"><p>使用“扫一扫”,即将网页分享至朋友圈。</p></div>
    </div>
    <div id="qrcode1" class="qrcode" style="display:none;">
        ...
        <div class="ftext" style="margin-top:-10px;"><p>使用“扫一扫”,即将网页分享给微信好友。</p></div>
    </div>
    ...
    <script id="navigator-slides-tpl" type="text/html">
        {{each datas item index}}
            ...
            <li id="slideItem-{{index}}"><a href="#navigator-location-{{index}}"><span>{{item}}</span></a><i class="dark" ></i></li>
            {{else}}
            <li id="slideItem-{{index}}"><a href="#navigator-location-{{index}}"><span>{{item}}（<font class="comment-total">0</font>）</span></a><i class="dark"></i></li>
    ...
    <script id="album-comments-tpl" type="text/html">
        ...
        <div class="pinglunview" data-id="{{comment.id}}">
            <div class="lpersonimage" id="lpersonimage_0" style="background-image: url(&quot;{{comment.author_avatar}}&quot;);"></div>
    ...
    <script id="album-similiars-tpl" type="text/html">
        {{each datas.data album album_index}}
        <div class="productview" >
            <div class="imageview">
                ...
                <div class="pimage" onclick="window.open('/album/s/{{album.web_id_code}}')" id="pimage_0" style="background-image: url(&quot;{{album.photo_cover}}&quot;);"></div>
                <div class="nametext">{{album.title}}</div>
                <div class="deview">
                    <div class="perimage" id="perimage_0" style="background-image: url(&quot;{{album.designer.detail.url_avatar}}&quot;);"></div>
                    <label class="pertext">{{album.designer.detail.nickname}}</label>
                    <div class="lookview">
            ...
            <div class="productcontainer" id="product">
                ...
                <div class="productview">
                    <div class="imageview">
                        ...
                        <div class="pimage" onclick="click_product('{{product.web_id_code}}')" id="primage_0" style="background-image: url(&quot;{{product.cover}}&quot;);"></div>
                        <div class="nametext">{{product.name}}</div>
                        <div class="companyview">
                            <label class="companytext">{{product.sales_name}}</label>
                            <label class="pricetext">{{product.sales_price}}</label>
                        </div>
                    </div>
                </div>
                {{/each}}
    ...
    <script id="designer-profile-tpl" type="text/html">
        <div class="personview">
            <div class="personimage" style="background-image: url('{{designer.url_avatar}}');"></div>
            <span class="personname">{{designer.nickname}}</span>
            <span class="company">{{if designer.organization}}所在组织：{{designer.organization}} {{/if}}</span>
    ...
    <script id="album-basic-info-tpl" type="text/html">
        <div class="right1">
            ...
            <div class="detailsview">
                ...
                {{if album.address_residential_quarter}}<span class="details1">小区：{{album.address_residential_quarter}}</span>{{/if}}
                {{if album.address_building}}<span class="details1">所在楼栋：{{album.address_building}}</span>{{/if}}
                {{if album.address_layout_number}}<span class="details1">所在户型号：{{album.address_layout_number}}</span>{{/if}}
            ...
            {{if album.description_layout}}<div class="details1view">设计说明：{{album.description_layout}}</div>{{/if}}
            <div class="likeview">
                <div class="likeview1">
                    <div class="block">
                        <span class="iconfont icon-dianzan" style="color:{{if album.liked}}#1582FF {{else}}#777777 {{/if}}" id="dianzan" onclick="like()"></span>
                        <span class="blocktext" id="dianzannumber">{{album.count_praise}}</span>
                    </div>
                    <div class="block">
                        <span class="iconfont icon-buoumaotubiao44" onclick="collect()" style="color:{{if album.collected}}#1582FF {{else}}#777777 {{/if}}" id="shoucang"></span>
                    ...
                    <div class="block share-hover-btn" onmouseover="pic()" onmouseout="pict()" onclick="pic()">
                        <span class="iconfont icon-share-fill" style="color:#777777;"></span>
                        <span class="blocktext">分享</span>
                        <div class="shareview1" style="display:none;" onmouseover="pic()" onmouseout="pict()">▲</div>
                        <div class="shareview" style="display:none;" onmouseover="pic()" onmouseout="pict()">
                            <img src="/v1/images/site/sjfa_xq/微信icon.png" class="weiicon" onclick="pic1()"/>
                            <img src="/v1/images/site/sjfa_xq/朋友圈icon.png" class="weiicon" style="margin-left:10px;"
                                 onclick="pic1t()"/>
                        </div>
                    </div>
                </div>
    ...
    <script id="album-sections-tpl" type="text/html">
            ...
            <div class="album-section-block navigator-location">
                ...
                <div class="productnav">
                    <div id="d_swiper_{{section_index}}_0" class="section-tab-title d_swipwer_span1" onclick="change_section_tab({{section_index}},0)"
                         style="width:104px;">空间设计
                    </div>
                    <div id="d_swiper_{{section_index}}_1" class="section-tab-title d_swipwer_span" onclick="change_section_tab({{section_index}},1)"
                         style="width:104px;">产品应用
                    </div>
                    <div id="d_swiper_{{section_index}}_2" class="section-tab-title d_swipwer_span" onclick="change_section_tab({{section_index}},2)" style="width:72px;">
                        施工
                    </div>
                </div>
                <div class="section-tab-content section-design" style="display:block">
                    <div class="image2view">
                        <a target="_blank" href="{{section.content.design.photos[0]}}">
                            ...
                            <div class="image2" id="image_{{section_index}}" src="{{section.content.design.photos[0]}}" style="background-image: url('{{section.content.design.photos[0]}}');"></div>
                            {{else}}
                        ...
                        <div class="detailsimageview" id="detailimage_1">
                            ...
                            <div class="img-thumb {{if image_index==0}}image4{{else}}image3{{/if}}" id="image_{{section_index}}_{{image_index}}" onmouseover="changepicture({{section_index}},'design',{{image_index}})" data-src="{{image_url}}" style="background-image: url('{{image_url}}');"></div>
                ...
                <div class="section-tab-content section-product">
                    <div class="image2view">
                        <a target="_blank" href="{{section.content.product.photos[0]}}">
                            ...
                            <div class="image2" id="image_{{section_index}}" src="{{section.content.product.photos[0]}}" style="background-image: url('{{section.content.product.photos[0]}}');"></div>
                            {{else}}
                        ...
                        <div class="detailsimageview" id="detailimage_1">
                            ...
                            <div class="img-thumb {{if image_index==0}}image4{{else}}image3{{/if}}" id="image_{{section_index}}_{{image_index}}" onmouseover="changepicture({{section_index}},'product',{{image_index}})" data-src="{{image_url}}" style="background-image: url('{{image_url}}');"></div>
                ...
                <div class="section-tab-content  section-build">
                    <div class="image2view">
                        <a target="_blank" href="{{section.content.build.photos[0]}}">
                            ...
                            <div class="image2" id="image_{{section_index}}" src="{{section.content.build.photos[0]}}" style="background-image: url('{{section.content.build.photos[0]}}');"></div>
                            {{else}}
                        ...
                        <div class="detailsimageview" id="detailimage_1">
                            ...
                            <div class="img-thumb {{if image_index==0}}image4{{else}}image3{{/if}}" id="image_{{section_index}}_{{image_index}}" onmouseover="changepicture({{section_index}},'build',{{image_index}})" data-src="{{image_url}}" style="background-image: url('{{image_url}}');"></div>
...
@section('script')
    ...
    <script>
        ...
        var album_section_api_url = "{{url('/album/api/list_album_sections/'.request()->route('id'))}}";
        var album_product_api_url = "{{url('/album/api/list_album_products/'.request()->route('id'))}}";
        var album_similiar_api_url = "{{url('/album/api/list_album_similiars/'.request()->route('id'))}}?__bs="+__cache_brand;
        var commit_comment_api_url = "{{url('/album/api/commit_comment/'.request()->route('id'))}}";
        var album_comments_api_url = "{{url('/album/api/list_album_comments/'.request()->route('id'))}}";
        var album_collect_api_url = "{{url('/album/api/collect')}}";
        var album_like_api_url = "{{url('/album/api/like')}}";
        var designer_focus_api_url = "{{url('/designer/api/focus')}}";
        var current_url = "{{url()}}";
