...
@section('content')
    ...
    <script type="text/html" id="tableToolbarTpl">
        ...
        @can('album_manage.album_index.hd_photo_album_detail')
        <a title="预览" target="_blank" href="/admin/seller/album/index/hd/seller_designer/preview_album_detail/@{{d.web_id_code}}"  class="layui-btn-custom-blue layui-btn  layui-btn-xs">
            <i class="layui-icon layui-icon-search"></i>
        </a>
        @endcan

        @can('album_manage.album_index.hd_photo_album_switch')
        @{{#     if( d.status_dealer=='1' ){       }}
        <a href="javascript:;" title="销售商主页下架" onclick="ajax_status('@{{d.changeStatusApiUrl}}')" class="layui-btn-danger layui-btn  layui-btn-xs" lay-event="edit">
            <i class="layui-icon layui-icon-close"></i>
        </a>
        @{{#     }else{     }}
        <a href="javascript:;" title="销售商主页展示" onclick="ajax_status('@{{d.changeStatusApiUrl}}')" class="layui-btn-success layui-btn  layui-btn-xs" lay-event="edit">
        ...
        @can('album_manage.album_index.hd_photo_album_apply_brand')
        @{{#     if( d.can_switch ){       }}
        <a href="javascript:;" title="@{{ d.status_brand_title }}" onclick="ajax_status('@{{d.changeStatusBrandApiUrl}}')" class="layui-btn-@{{ d.status_brand_style }} layui-btn  layui-btn-xs" lay-event="edit">
            @{{ d.status_brand_title }}
        </a>
        @{{#     }     }}
...
@section('script')
    ...
    <script>
        ...
        $(function () {
            ...
            layui.use(['jquery','tableSelect'], function () {
                ...
                tableSelect.render({
                    ...
                    table: {
                        url:'{!! url($url_prefix.'admin/seller/album/index/hd/seller_designer/api/get_brands') !!}',
                        cols: [[
                            { type: 'radio' },
        ...
        let base_api = '{{url('/admin/seller/album/index/hd/seller_designer/api')}}';
        let table_api = '{{url('/admin/seller/album/index/hd/seller_designer/api')}}';
        //转换静态表格
        ...
        let tableInstance = table.render({
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'status_dealer_text',width:100, title: '展示销售商主页'}
    ...
    <script>
        ...
        function openDetail(id){
            let page = '{{url('/admin/seller/album/index/hd/seller_designer')}}/'+id;
            layer.open({
                type: 2,
                title:'方案详情',
                area:['900px', '600px'],
