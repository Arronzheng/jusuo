...
@section('content')
    <div class="layui-tab layui-tab-brief"  lay-filter="docDemoTabBrief">
        <ul class="layui-tab-title">
            <li onclick="location.href='{{url($url_prefix.'admin/seller/security_center/bind_wechat')}}'" class="layui-this">绑定微信</li>
            <li onclick="location.href='{{url($url_prefix.'admin/seller/security_center/modify_pwd')}}'" >修改密码</li>
        </ul>
        <div class="layui-tab-content" >
            <div class="bind-container">
                @if(!Auth::guard('seller')->user()->login_wx_openid)
...
@section('script')
    <script>
        ...
        form.on('submit(confirmPwd)', function(form_info){
            ...
            ajax_post('{{url($url_prefix.'admin/seller/security_center/api/verify_pwd')}}',
                form_field,
                function(result){
                    if(result.status){
                        layer.msg('密码验证成功');
        ...
        function showQrcode() {
            ...
            $('#qrcode').qrcode({width:150,height:150,text:"{{ url('bind',Auth::guard('seller')->user()->remember_token) }}?type={{\App\Http\Services\common\WechatService::SELLER}}"});
        }
        ...
        function check_wechat_bind(){
            ...
            ajax_post('{{url('check_wechat_bind')}}',{'t':'{{ Auth::guard('seller')->user()->remember_token }}','openid':'{{ Auth::guard('seller')->user()->login_wx_openid }}','type':'{{\App\Http\Services\common\WechatService::SELLER}}'},function (json) {
                if (json.status) {
                    layer.msg('绑定成功！');
                    location.reload();
                }
