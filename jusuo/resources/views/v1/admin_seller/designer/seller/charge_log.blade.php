...
@section('script')
    ...
    <script>
        ...
        form.render();
        let designer_id = '{{$id}}';
        let base_api = '{{url('/admin/seller/seller_designer/api/charge_log')}}/'+designer_id;
        let table_api = '{{url('/admin/seller/seller_designer/api/charge_log')}}/'+designer_id;
        //转换静态表格
        ...
        let tableInstance = table.render({
            ...
            ,cols: [[ //表头'order_no','realname','mobile','money','integral','pay_time','pay_time'
                {field: 'id', title: 'ID'}
                ,{field: 'order_no',title: '系统订单号'}
                ,{field: 'money', title: '充值金额'}
                ,{field: 'integral', title: '获得积分数量'}
                ,{field: 'pay_time', title: '支付时间'}
                ,{field: 'pay_no', title: '支付交易号'}
            ]]
        });
