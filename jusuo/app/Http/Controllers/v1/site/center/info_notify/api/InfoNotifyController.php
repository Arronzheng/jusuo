...
namespace App\Http\Controllers\v1\site\center\info_notify\api;
...
class InfoNotifyController extends ApiController
{
    ...
    public function index(Request $request)
    {
        ...
        $dealer_id = $loginAdmin->dealer->id;
        $input = $request->all();

        $entry = MsgProductCeramicDealer::query();
        $entry->where('dealer_id',$dealer_id)
            ->orderBy('id','desc');
        ...
        $datas->transform(function($v) use($loginAdmin){
            if($loginAdmin->is_super_admin){
                $v->type_text = MsgProductCeramicDealer::typeGroup($v->type);
                $v->is_read_text = MsgProductCeramicDealer::isReadGroup($v->is_read);
            }else{
                $v->type_text = MsgProductCeramicDealer::typeGroup($v->type);
                $v->is_read_text = MsgProductCeramicDealer::isReadGroup($v->is_read);
            }
            $v->setReadApiUrl = url('admin/seller/info_notify/api/product_notify/'.$v->id.'/set_read');
            return $v;
        });
    ...
    public function set_read($id, Request $request)
    {
        ...
        try{
            ...
            $msg = $entry->find($id);
            $msg->is_read = MsgProductCeramicDealer::IS_READ_YES;
            $msg->save();
