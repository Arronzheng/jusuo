...
namespace App\Http\Controllers\v1\admin\brand\info_statistics\account\designer\api;
...
class AccountController extends ApiController
{
    ...
    public function account_index(Request $request)
    {
        ...
        $organization_id = $request->input('orgid',null);
        $range_delta = $request->input('range_delta',1);

        if(!in_array($range_delta,array(1,7,30))){
            return json_encode([]);
        }
        ...
        if($organization_id!==null){
            $entry = $entry->where('designers.organization_type',Designer::ORGANIZATION_TYPE_SELLER);
            $entry = $entry->where('designers.organization_id',$organization_id);
        }
        ...
        if($name!==null){
            $entry = $entry->where(function($query)use($name){
                $query->where('designers.login_username','like',"%".$name."%");
                $query->orWhere('designers.login_mobile','like',"%".$name."%");
        ...
        if($reg_start!==null && $reg_end!==null){
            $entry->whereBetween('designers.created_at', array($reg_start.' 00:00:00', $reg_end.' 23:59:59'));
        }
        ...
        $entry->join('designer_details as detail','detail.designer_id','=','designers.id')
            ->leftJoin('log_designer_certifications as ldc','ldc.target_designer_id','=','designers.id')
            ...
            ->select([
                ...
                'designers.top_brand_status',
                'designers.organization_type',
                'designers.organization_id',
                'designers.status as account_status',
                ...
                'detail.count_visit',
                'detail.count_praise',
                'stat.count_upload_album_day_'.$range_delta,
                'stat.count_top_album_day_'.$range_delta,
                'stat.count_fav_album_day_'.$range_delta,
                'stat.count_praise_album_day_'.$range_delta,
                'stat.count_download_album_day_'.$range_delta,
                'stat.count_copy_album_day_'.$range_delta,
                'stat.count_fav_designer_day_'.$range_delta,
                'stat.count_faved_designer_day_'.$range_delta,
                'ldc.is_approved as cert_status',
                'ldc.approve_time',
            ])
        ...
        //关联擅长风格
        if($style_id!=null){
            $entry->join('designer_styles as dstyle','dstyle.designer_id','=','designers.id');
            $entry->where('dstyle.style_id',$style_id);
        }

        //关联擅长空间
        if($space_id!=null){
            $entry->join('designer_spaces as dspace','dspace.designer_id','=','designers.id');
            $entry->where('dspace.space_id',$space_id);
        }

        $entry->orderBy('stat.id','desc');

        $datas = $entry->paginate($limit);

        $datas->transform(function($v)use($range_delta){
            ...
            $v->changeStatusApiUrl = url('admin/brand/info_statistics/account/designer/api/'.$v->designer_id.'/top_status');
            //所属组织
            $v->organization = '无';
            ...
            'stat.count_faved_designer_day_'.$range_delta,
             */
            $count_upload_album_column = 'count_upload_album_day_'.$range_delta;
            $count_top_album_column = 'count_top_album_day_'.$range_delta;
            $count_fav_album_column = 'count_fav_album_day_'.$range_delta;
            $count_praise_album_column = 'count_praise_album_day_'.$range_delta;
            $count_download_album_column = 'count_download_album_day_'.$range_delta;
            $count_copy_album_column = 'count_copy_album_day_'.$range_delta;
            $count_fav_designer_column = 'count_fav_designer_day_'.$range_delta;
            $count_faved_designer_column = 'count_faved_designer_day_'.$range_delta;
            $v->count_upload_album = $v->$count_upload_album_column;
            $v->count_top_album = $v->$count_top_album_column;
            $v->count_fav_album = $v->$count_fav_album_column;
            $v->count_praise_album = $v->$count_praise_album_column;
            $v->count_download_album = $v->$count_download_album_column;
            $v->count_copy_album = $v->$count_copy_album_column;
            $v->count_fav_designer = $v->$count_fav_designer_column;
            $v->count_faved_designer = $v->$count_faved_designer_column;
            return $v;
        });
    ...
    public function change_status($id, Request $request)
    {
        ...
        $designer = Designer::query()
             ->where(function($query1) use($brand_id){
                 //筛选品牌或销售商设计师
                 $query1->where(function($brand_designer)use($brand_id){
                     $brand_designer->where('organization_type',Designer::ORGANIZATION_TYPE_BRAND);
                     $brand_designer->whereHas('brand',function($organization)use($brand_id){
        ...
        try{
            ...
            //更新状态
            if($designer->top_brand_status==Designer::TOP_BRAND_STATUS_OFF){
                $designer->top_brand_status = Designer::TOP_BRAND_STATUS_ON;
                $designer->top_brand_time = Carbon::now();
                ...
                $designer->top_brand_status = Designer::TOP_BRAND_STATUS_OFF;
            }
