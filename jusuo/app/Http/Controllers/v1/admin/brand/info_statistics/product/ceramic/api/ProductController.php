...
namespace App\Http\Controllers\v1\admin\brand\info_statistics\product\ceramic\api;
...
class ProductController extends ApiController
{
    ...
    public function index(Request $request)
    {
        ...
        $entry->select(['id','type','parent_id','name','brand_id','code','sys_code',
            ...
            'created_at','status','top_status_brand'
        ]);

        //所属品牌
        $entry->whereHas('brand',function($query)use($brand_id){
        ...
        $datas->transform(function($v){
            ...
            $v->changeStatusApiUrl = url('admin/brand/info_statistics/product/ceramic/api/'.$v->id.'/top_status');
    ...
    public function change_status($id, Request $request)
    {
        ...
        try{
            ...
            //更新状态
            if($data->top_status_brand==ProductCeramic::TOP_PLATFORM_OFF){
                $data->top_status_brand = ProductCeramic::TOP_PLATFORM_ON;
            }else{
                $data->top_status_brand = ProductCeramic::TOP_PLATFORM_OFF;
            }
