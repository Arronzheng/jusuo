...
namespace App\Http\Controllers\v1\admin\brand\info_statistics\album\api;
...
class AlbumController extends ApiController
{
    ...
    public function index(Request $request)
    {
        ...
        $entry = Album::query()
            ->select([
                'albums.id','albums.web_id_code','albums.id as album_id','albums.code','albums.type','albums.title','albums.count_area',
                ...
                'albums.top_status_brand','albums.designer_id','albums.period_status'
            ])
            ->with(['designer:id,designer_account','designer.detail:id,designer_id,realname,nickname,area_belong_province,area_belong_city'])
        ...
        $datas->transform(function($v){
            ...
            $v->nickname = isset($v->designer->detail->nickname)?$v->designer->detail->nickname:'';
            $v->designer_account = isset($v->designer->designer_account)?$v->designer->designer_account:'';

            $v->can_change_status = $v->period_status==Album::PERIOD_STATUS_FINISH?1:0;
            $v->changeStatusApiUrl = url('admin/brand/info_statistics/album/api/'.$v->id.'/top_status');

            //设计师所在城市
            //$v->belong_city = DesignerService::getDesignerBelongArea($v->designer->id);
            $v->belong_city = '';
            $province =  Area::where('id',$v->designer->detail->area_belong_province)->first();
            $city =  Area::where('id',$v->designer->detail->area_belong_city)->first();
            //$district =  Area::where('id',$v->designer->detail->area_belong_district)->first();
            if($province){$v->belong_city.= $province->name;}
            if($city){$v->belong_city.= $city->name;}
            //if($district){$v->belong_city.= $district->name;}
            unset($v->designer);
            return $v;
        });
    ...
    public function change_status($id, Request $request)
    {
        ...
        try{
            ...
            //更新状态
            if($album->top_status_brand==Album::TOP_BRAND_OFF){
                $album->top_status_brand = Album::TOP_BRAND_ON;
            }else{
                $album->top_status_brand = Album::TOP_BRAND_OFF;
