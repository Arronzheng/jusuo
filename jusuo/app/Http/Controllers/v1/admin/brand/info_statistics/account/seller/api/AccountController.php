...
namespace App\Http\Controllers\v1\admin\brand\info_statistics\account\seller\api;
...
class AccountController extends ApiController
{
    ...
    public function account_index(Request $request)
    {
        ...
        $entry->join('organization_dealers as d','as.dealer_id','=','d.id')
            ...
            ->select([
                ...
                'd.id as seller_id','d.name as company_name','d.p_brand_id','d.p_dealer_id','d.level',
                'dd.area_belong_id','dd.area_serving_city','dd.point_focus','dd.star_level',
                'd.status as seller_status','d.top_brand_status',
                'b.product_category','b.name as brand_company_name','b.id as brand_id',
                'stat.count_product_increase_day_'.$range_delta,'stat.count_album_increase_day_'.$range_delta,'stat.count_designer_increase_day_'.$range_delta,
                'stat.count_product','stat.count_album','stat.count_designer',
        ...
        $datas->transform(function($v)use($range_delta){
            //二级销售商的产品数跟一级销售商
            if($v->level==2){
                $parent_dealer_id = $v->p_dealer_id;
                $parent_dealer_detail = DetailDealer::where('dealer_id',$parent_dealer_id)->first();
                if($parent_dealer_detail){
                    $v->count_product = $parent_dealer_detail->count_product;
                }
            }
            ...
            $v->changeStatusApiUrl = url('admin/brand/info_statistics/account/seller/api/'.$v->seller_id.'/top_status');
            $v->area_belong_text = '';
            if ($v->area_belong_id){
                ...
                if ($district){
                    ...
                    if ($city){
                        ...
                        if ($province){
                            ...
                            $v->area_belong_text = $province->name.'/'.$city->name;
                        }
                    }
                }
            ...
            if ($v->area_serving_city){
                $cityIds = explode(DealerService::JOINER,$v->area_serving_city);
                if(count($cityIds)>0){
                    $area_serving_citys = Area::where('level',2)->whereIn('id',$cityIds)->get()->pluck('shortname')->toArray();
                    if(count($area_serving_citys)>0){
                        $v->area_serving_text = implode(',',$area_serving_citys);
                    }
                }
            }
            $count_product_increase_column = 'count_product_increase_day_'.$range_delta;
            $count_album_increase_column = 'count_album_increase_day_'.$range_delta;
            $count_designer_increase_column = 'count_designer_increase_day_'.$range_delta;
            $v->count_product_increase = $v->$count_product_increase_column;
            $v->count_album_increase = $v->$count_album_increase_column;
            $v->count_designer_increase = $v->$count_designer_increase_column;
            return $v;
        });
    ...
    public function change_status($id, Request $request)
    {
        ...
        try{
            ...
            //更新状态
            if($seller->top_brand_status==OrganizationDealer::TOP_BRAND_STATUS_OFF){
                $seller->top_brand_status = OrganizationDealer::TOP_BRAND_STATUS_ON;
                $seller->top_brand_time = Carbon::now();
            }else{
                $seller->top_brand_status = OrganizationDealer::TOP_BRAND_STATUS_OFF;
            }
