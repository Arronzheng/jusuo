...
namespace App\Http\Controllers\v1\admin\brand\integral\goods\api;
...
class IntegralGoodsController extends ApiController
{
    ...
    public function index(Request $request)
    {
        ...
        $entry = IntegralGood::query()
            ->where(function($query) use($loginBrand){
                //品牌自建商品
                $query->where('brand_id',$loginBrand->id);
                //品牌已引入平台的商品
                $query->orWhere(function($query1)use($loginBrand){
                    $query1->whereHas('used_brands',function($brand)use($loginBrand){
                        $brand->where('organization_brands.id',$loginBrand->id);
                    });
                });
            });
        ...
        $entry->select([
            'integral_goods.id','name','web_id_code','market_price','integral','integral_brand_id','brand_id','category_id_1','category_id_2',
        ...
        $datas->transform(function($v)use($loginBrand){
            ...
            if($v->is_import){
                ...
                if($authorization->visible_status == IntegralGoodAuthorization::VISIBLE_STATUS_ON){
                    $v->statusIsOn = true;
                }
                if($authorization->top_status == IntegralGoodAuthorization::TOP_STATUS_ON){
                    $v->isTop = true;
                }
                $v->status_text = IntegralGoodAuthorization::visibleStatusGroup($authorization->visible_status);
                $v->top_status_text = IntegralGoodAuthorization::topStatusGroup($authorization->top_status);
                $v->changeStatusApiUrl = url('admin/brand/integral/goods/api/'.$v->id.'/auth_status');
                $v->changeTopApiUrl = url('admin/brand/integral/goods/api/'.$v->id.'/auth_top');
                $v->deleteApiUrl = '/admin/brand/integral/goods/api/'.$v->id.'/auth';
            }else{

                if($v->status == IntegralGood::STATUS_ON){
                    $v->statusIsOn = true;
                }
                if($v->top_status == IntegralGood::TOP_STATUS_ON){
                    $v->isTop = true;
                }
                $v->status_text = IntegralGood::statusGroup($v->status);
                $v->top_status_text = IntegralGood::topStatusGroup($v->top_status);
                $v->changeStatusApiUrl = url('admin/brand/integral/goods/api/'.$v->id.'/status');
                $v->changeTopApiUrl = url('admin/brand/integral/goods/api/'.$v->id.'/top');
                $v->deleteApiUrl = '/admin/brand/integral/goods/api/'.$v->id;
            }
            return $v;
        });
    ...
    public function update(Request $request)
    {
        ...
        $data = IntegralGood::query()->where('brand_id',$loginBrand->id)->find($input_data['id']);
    ...
    public function change_status($id, Request $request)
    {
        ...
        $data = IntegralGood::where('brand_id',$loginBrand->id)->find($id);
        if(!$data){
            $this->respFail('数据不存在');
        }
    ...
    //更改开放表上下架状态
    public function change_auth_status($id, Request $request)
    {
        $loginAdmin = $this->authService->getAuthUser();
        $loginBrand = $loginAdmin->brand;
        ...
        try{
            ...
            //更新状态
            if($authorization->visible_status==IntegralGoodAuthorization::VISIBLE_STATUS_OFF){
                $authorization->visible_status = IntegralGoodAuthorization::VISIBLE_STATUS_ON;
            }else{
                $authorization->visible_status = IntegralGoodAuthorization::VISIBLE_STATUS_OFF;
            }
    ...
    //更改开放表置顶
    public function change_auth_top($id, Request $request)
    {
        $loginAdmin = $this->authService->getAuthUser();
        $loginBrand = $loginAdmin->brand;
        ...
        try{
            ...
            //更新状态
            if($authorization->top_status==IntegralGoodAuthorization::TOP_STATUS_OFF){
                $authorization->top_status = IntegralGoodAuthorization::TOP_STATUS_ON;
                $authorization->top_time = Carbon::now();
            }else{
                $authorization->top_status = IntegralGoodAuthorization::TOP_STATUS_OFF;
            }
    ...
    //删除
    public function destroy_auth(Request $request)
    {
        $input_data = $request->all();
        ...
        //删除暂时定为取消引入
        $authorization->status = IntegralGoodAuthorization::STATUS_BRAND_CAN_USE;
        $authorization->save();
    ...
    public function upload_video(Request $request)
    {
        ...
        if($access_url = $service->simple_upload(UploadOssService::KEY_DIR_PRODUCT_VIDEO."/brand/")){
            $this->respData([
                'access_path'=>$service->result['data']['access_path'],
                'base_path'=>$service->result['data']['base_path'],
            ]);
