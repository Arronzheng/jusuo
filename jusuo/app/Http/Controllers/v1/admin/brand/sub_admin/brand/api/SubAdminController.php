...
namespace App\Http\Controllers\v1\admin\brand\sub_admin\brand\api;
...
class SubAdminController extends ApiController
{
    ...
    public function account_index(Request $request)
    {
        ...
        $entry = AdministratorBrand::query();
        ...
        $datas->transform(function($v)use($loginAdmin){
            ...
            $v->isOn = $v->status==AdministratorBrand::STATUS_ON;
            $v->canEdit = 0;
            ...
            $v->changeStatusApiUrl = url('admin/brand/sub_admin/api/account/'.$v->id.'/status');
            return $v;
        });
    ...
    public function modify_pwd(Request $request)
    {
        ...
        if($loginAdmin->is_super_admin || $loginAdmin->id==$input_data['id']){
            //超级管理员修改自己的密码或者所有管理员修改自己的密码时
            $validateRules['old_password'] = 'required';
        }

        $validator = Validator::make($input_data, $validateRules);
