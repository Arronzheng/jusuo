...
namespace App\Http\Controllers\v1\admin\brand\product\verify\wait;
...
class WaitVerifyController extends VersionController
{
    private $authService;
    ...
    public function index(Request $request)
    {
        ...
        return $this->get_view('v1.admin_brand.product.verify.wait.index',compact('vdata'));
    }
    ...
    public function detail($id)
    {
        ...
        $data->parent_product = '';
        if($data->type==ProductCeramic::TYPE_ACCESSORY && isset($content['parent_id'])){
            $parent_product = ProductCeramic::find($content['parent_id']);
            if($parent_product){
                $data->parent_product = $parent_product->name;
            }
        }
        ...
        //指导价
        $data->guide_price = isset($content['guide_price'])?$content['guide_price']:'';
        //产品编码
        $data->code = isset($content['code'])?$content['code']:'';
        //产品结构
        $data->structure_text = '';
        if(isset($content['structure_id'])){
            $structure = ProductCeramicStructure::where('brand_id',$brand->id)->find($content['structure_id']);
            if($structure){$data->structure_text = $structure->name;}
        }
        //系列
        $data->spec_text = '';
        ...
        if(isset($content['collocations'])){
            ...
            for($i=0;$i<count($collocations);$i++){
                if(isset($collocations[$i]['collocation_id']) && $collocations[$i]['collocation_id']>0){
        ...
        return $this->get_view('v1.admin_brand.product.verify.wait.detail',compact(
            'data'
        ));
    ...
    public function create()
    {
        ...
        return $this->get_view('v1.admin_brand.product.verify.wait.edit',compact('vdata','cf'));
    }
