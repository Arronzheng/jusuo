...
class ProductController extends ApiController
{
    ...
    public function index(Request $request)
    {
        ...
        //20200704增加，只看自己上传的产品
        $entry->where('create_by_administrator_id',$loginAdmin->id);
        ...
        $datas->transform(function($v)use($brand){
            ...
            /*$v->structure_text = '';
            $structure = $v->structure_id?ProductCeramicStructure::find($v->structure_id):null;
            if($structure){
                $v->structure_text = $structure->name;
            }*/
            //方案数
            $v->album_counts = $v->albums()->count();
            $v->changeStatusApiUrl = url('admin/brand/product/api/'.$v->id.'/status');
            $v->changeTopStatusApiUrl = url('admin/brand/info_statistics/product/ceramic/api/'.$v->id.'/top_status');
            $v->can_switch = $v->has_first_approved==ProductCeramic::HAS_FIRST_APPROVED_YES?1:0;
            $v->can_preview = $v->has_first_approved==ProductCeramic::HAS_FIRST_APPROVED_YES?1:0;
            $v->can_delete = false;
            if($v->status == ProductCeramic::STATUS_TEMP){
                $has_log = LogProductCeramic::where('target_product_id',$v->id)->count();
                if(!$has_log){
                    $v->can_delete = true;
                }
            }
            ...
            if($v->status == ProductCeramic::STATUS_PASS){
                ...
                DB::enableQueryLog();
                $usage = $v->authorize_dealer_on->count();
                $v->seller_usage = $usage;
                ...
                if($log){
                    ...
                    /*$v->structure_text = '';
                    $structure = isset($content['structure_id'])?ProductCeramicStructure::find($content['structure_id']):null;
                    if($structure){
                        $v->structure_text = $structure->name;
                    }*/
                    ...
                    $v->image = '';
                    //获取第一个产品图作为缩略图
                    if(isset($content['photo_product'])&&isset($content['photo_product'][0]) && $content['photo_product'][0]){
                    ...
                    if(isset($content['apply_categories'])){
                        $apply_category_ids = array_values($content['apply_categories']);
                        if(is_array($apply_category_ids) && isset($apply_category_ids[0]) && $apply_category_ids[0]) {
                    ...
                    if(isset($content['technology_categories'])){
                        $technology_category_ids = array_values($content['technology_categories']);
                        if(is_array($technology_category_ids) && isset($technology_category_ids[0]) && $technology_category_ids[0]) {
                    ...
                    if(isset($content['surface_features'])){
                        $surface_features_ids = array_values($content['surface_features']);
                        if(is_array($surface_features_ids) && isset($surface_features_ids[0]) && $surface_features_ids[0]) {
                    ...
                    if(isset($content['colors'])){
                        $colors_ids = array_values($content['colors']);
                        if(is_array($colors_ids) && isset($colors_ids[0]) && $colors_ids[0]) {
                    ...
                    if(isset($content['styles'])){
                        $styles_ids = array_values($content['styles']);
                        if(is_array($styles_ids) && isset($styles_ids[0]) && $styles_ids[0]) {
    ...
    private function export($datas)
    {
        ...
        $result = [
            [
                'id','名称','指导价','类型','产品编号','系统编号','规格','系列','应用类别','工艺类别','表面特征',
                '色系','定价方式','可应用空间风格','关联方案数','浏览量','使用量','收藏量','关注度','创建时间','显示状态',
                '品牌站可用状态'
            ]
        ];
        ...
        foreach($datas as $v){
            ...
            //$resultItem[] = $v->structure_text;
            $resultItem[] = $v->album_counts;
    ...
    public function store(Request $request)
    {
        ...
        $check_result = $this->product_save_check($input_data);
        if($check_result['status']==0){
            return $this->respFailReturn($check_result['msg']);
        }
        ...
        try{
            ...
            $log = $this->new_product_ceramic_log($input_data,$data);
    ...
    public function temp_save(Request $request)
    {
        ...
        try{
            ...
            $data->create_by_administrator_id = $loginAdmin->id;
            $data->type = $input_data['type'];
            $data->status = ProductCeramic::STATUS_TEMP;  //暂存
            //暂存时前端不能显示
            $data->visible = ProductCeramic::VISIBLE_NO;
            $data->save();

            $log = $this->new_temp_log($input_data,$data);
    ...
    //产品暂存更新
    public function temp_update(Request $request,$log_id)
    {
        $input_data = $request->all();
        $loginAdmin = $this->authService->getAuthUser();
        $brand = $loginAdmin->brand;
        ...
        //check
        /*$check_result = $this->product_save_check($input_data,true);
        if($check_result['status']==0){
        ...
        try{
            ...
            $new_log = $this->new_temp_log($input_data,$product);
            ...
            $product->status = ProductCeramic::STATUS_TEMP;  //暂存
            //暂存时前端不能显示
    ...
    public function update(Request $request)
    {
        ...
        try{
            ...
            //编辑提交就是修改审核，比如暂存之后的首次提交，就是首次审核
            //需要通过判断是否有过审核log来判定
            $log_type = LogProductCeramic::TYPE_FIRST_VERIFY;

            $exist_log = LogProductCeramic::query()
                ->where('target_product_id',$product->id)
                ->where('brand_id',$brand->id)
                ->first();
            if($exist_log){
                $log_type =LogProductCeramic::TYPE_MODIFY_VERIFY;
            }

            //如果产品已有待审核的修改审核log，则需要作废
            $modify_log = LogProductCeramic::query()
                ->where('target_product_id',$product->id)
                ...
                ->first();
            if($modify_log){
                $modify_log->is_approved = LogProductCeramic::IS_APROVE_CANCEL;
                $modify_log->save();
            }

            //如果产品已有暂存的log，则删除
            $temp_log = TmpProductCeramic::query()
                ->where('target_product_id',$product->id)
                ->where('brand_id',$brand->id)
                ->first();
            if($temp_log){
            ...
            //生成新的修改审核待审核log
            $new_log = $this->new_product_ceramic_log($input_data,$product,$log_type);

            //更改产品状态为待审核
            $product->status = ProductCeramic::STATUS_VERIFYING;
            if($log_type == LogProductCeramic::TYPE_FIRST_VERIFY){
                //首次审核，前端还不能显示（首次审核不通过，之后再提交审核，也算是首次审核）
                $product->visible = ProductCeramic::VISIBLE_NO;
            }
    ...
    //产品保存的check参数
    private function product_save_check($input_data,$is_edit=false)
    {
        $result = [
            'status' => 1,
            'msg' => 'success',
        ...
        $messages = [
            ...
            //'structure_id.required' => '请选择默认产品结构',
        ...
        //判断产品结构（20200323不填写默认产品结构）
        /*$exist_structure = ProductCeramicStructure::where('brand_id',$brand->id)->find($input_data['structure_id']);
        if(!$exist_structure){
            $result['status'] = 0;
            $result['msg'] = '产品结构不存在！';
            return $result;
    ...
    private function new_product_ceramic_log($input_data,$product,$type=LogProductCeramic::TYPE_FIRST_VERIFY)
    {
        ...
        $log_content = $this->get_log_content($input_data,$brand,$product);
    ...
    //生成一个新的暂存log
    private function new_temp_log($input_data,$product)
    {
        $loginAdmin = $this->authService->getAuthUser();
        $brand = $loginAdmin->brand;

        $log_content = $this->get_tmp_log_content($input_data,$brand,$product);

        $log = new TmpProductCeramic();
        $log->brand_id = $brand->id;
        $log->target_product_id = $product->id;
        $log->content = $log_content;
        $log->save();
    ...
    private function issetContent($content){
        return isset($content)?$content:'';
    }

    //生成tmp_log表中的content字段内容
    private function get_tmp_log_content($input_data,$brand,$product)
    {
        //生成一个新的待审核log
        $log_content = array();
        $log_content['name'] = $this->issetContent($input_data['name']);
        $log_content['guide_price'] = 0;
        $log_content['code'] = $this->issetContent($input_data['code']);
        //$log_content['structure_id'] = $input_data['structure_id'];
        $log_content['type'] = $this->issetContent($input_data['type']);
        $log_content['parent_id'] = $this->issetContent($input_data['parent_id']);
        if(isset($input_data['type'])&&isset($input_data['parent_id'])&&isset($input_data['series']))
        ...
        if(isset($input_data['physical_chemical_property'])){
            $log_content['physical_chemical_property'] = serialize($this->issetContent($input_data['physical_chemical_property']));
        }
        if(isset($input_data['function_feature'])){
            $log_content['function_feature'] = serialize($this->issetContent($input_data['function_feature']));
        }
    ...
    //生成log表中的content字段内容
    private function get_log_content($input_data,$brand,$product)
    {
        //生成一个新的待审核log
        $log_content = array();
    ...
    public function destroy(Request $request)
    {
        ...
        try{
            ...
            $data = ProductCeramic::find($inputData['id']);
            if(!$data){
                $this->respFail('信息不存在！');
            }

            if($data->status != ProductCeramic::STATUS_TEMP){
                $this->respFail('状态错误，无法删除！');
            }

            $has_log = LogProductCeramic::where('target_product_id',$data->id)->count();
            if($has_log>0){
                $this->respFail('状态错误，无法删除！');
            }

            //删除temp log
            $tmp_delete = $tmp_log = TmpProductCeramic::where('target_product_id',$data->id)->delete();

            $result = $data->delete();

            if(!$tmp_delete || !$result){
                DB::rollback();
                $this->respFail('操作错误！');
            }
