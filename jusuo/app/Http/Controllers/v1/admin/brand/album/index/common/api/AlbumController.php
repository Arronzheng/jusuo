...
namespace App\Http\Controllers\v1\admin\brand\album\index\common\api;
...
class AlbumController extends ApiController
{
    ...
    public function index(Request $request)
    {
        ...
        $login_brand_id = $brand->id;
        $entry = Album::query()
            ->select([
                DB::raw('(select count(*) from album_space_types as abs where abs.album_id = albums.id) as space_count'),
                'albums.id','type','title','code','albums.web_id_code','status','status_brand','designer_id','updated_at','count_area','visible_status','status_platform'
            ])
            ->whereHas('designer',function($query1)use($login_brand_id,$designer_type){
                //设计师类型
                if($designer_type!==null){
                    if($designer_type==1){
                        $query1->where(function($brand_designer)use($login_brand_id){
                            $brand_designer->where('organization_type',Designer::ORGANIZATION_TYPE_BRAND);
                            $brand_designer->where('organization_id',$login_brand_id);
                        });                    }
                    if($designer_type==2){
                        $query1->where(function($seller_designer)use($login_brand_id){
                            $seller_designer->where('organization_type',Designer::ORGANIZATION_TYPE_SELLER);
                            $seller_designer->whereHas('seller',function($organization)use($login_brand_id){
                                $organization->where('organization_dealers.p_brand_id',$login_brand_id);
                            });
                        });
                    }
                }else{
                    ...
                    $query1->orWhere(function($seller_designer)use($login_brand_id){
                        $seller_designer->where('organization_type',Designer::ORGANIZATION_TYPE_SELLER);
                        $seller_designer->whereHas('seller',function($organization)use($login_brand_id){
                            $organization->where('organization_dealers.p_brand_id',$login_brand_id);
                        });
        ...
        $entry->where('status',Album::STATUS_PASS); //必须是已审核通过的

        $entry->orderBy('id','desc');

        $datas =$entry->paginate(intval($limit));

        $codeStatusBrandVerifying = Album::STATUS_BRAND_VERIFYING;
        $codeStatusBrandOff = Album::STATUS_BRAND_OFF;

        $datas->transform(function($v)use($codeStatusBrandVerifying,$codeStatusBrandOff){
            ...
            $v->status_brand_text = Album::statusBrandGroup($v->status_brand?$v->status_brand:'');
            $v->status_text = Album::statusGroup($v->status);
            $v->can_switch_status_brand = false;
            $v->designer_type = '销售商设计师';
            $v->designer_type_code = 1;
            $v->changeStatusApiUrl = '';
            $v->approveStatusApiUrl = url('admin/brand/album/verify/hd/seller_apply_show/api/'.$v->id.'/approval');
            $v->rejectStatusApiUrl = url('admin/brand/album/verify/hd/seller_apply_show/api/'.$v->id.'/reject');
            //品牌方案的品牌展示上下架
            if($v->designer->organization_type == Designer::ORGANIZATION_TYPE_BRAND){
                $v->designer_type = '品牌设计师';
                $v->designer_type_code = 2;
                $v->changeStatusApiUrl = url('admin/brand/album/index/common/api/'.$v->id.'/status');
            }
            $v->codeStatusBrandVerifying = $codeStatusBrandVerifying;
            $v->codeStatusBrandOff = $codeStatusBrandOff;
            //平台站状态
            ...
            switch($v->status_platform){
                ...
                case Album::STATUS_PLATFORM_OFF:
                    ...
                    $v->changeStatusPlatformApiUrl = url('admin/brand/album/index/hd/brand_designer/api/'.$v->id.'/status_platform');
                    break;
    ...
    public function change_status_platform($id, Request $request)
    {
        ...
        if(
            $designer->organization_type != Designer::ORGANIZATION_TYPE_BRAND ||
            $designer->organization_id != $brand->id
        ){
            $this->respFail('权限不足！');
        }
