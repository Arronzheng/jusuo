...
namespace App\Http\Controllers\v1\admin\platform\info_verify\designer_basic\api;
...
class DesignerController extends ApiController
{
    ...
    public function approve_designer($log, $verify_content)
    {
        ...
        $result = $this->basicInfoService->approve_basic_info(0,Designer::ORGANIZATION_TYPE_NONE,$user->id,$log,$verify_content);
        if($result['status']==1){
            $this->respData([]);
        }else{
