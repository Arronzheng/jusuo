...
namespace App\Http\Controllers\v1\admin\platform\info_verify\designer_certification\api;
...
class DesignerController extends ApiController
{
    ...
    public function account_index(Request $request)
    {
        ...
        $datas = $entry->select(
            ...
            'log_designer_certifications.content',
        ...
        $datas->transform(function($v){
            ...
            $v->realname = isset($content['legal_person_name'])?$content['legal_person_name']:'';
            $v->code_idcard = isset($content['code_idcard'])?$content['code_idcard']:'';
            ...
            $v->designer_type_text= DesignerDetail::designerTypeGroup($v->self_designer_type==null?'':$v->self_designer_type);
    ...
    public function verify_approval($id)
    {
        ...
        $log = LogDesignerCertification::query()
            ->whereHas('target_designer',function($query){
                $query->where('organization_type',Designer::ORGANIZATION_TYPE_NONE);
                $query->has('detail');
            })
            ->find($id);
        ...
        $result = $this->appInfoService->approve_app_info(0,Designer::ORGANIZATION_TYPE_NONE,$user->id,$log,$verify_content);
        if($result['status']==1){
            $this->respData([]);
        }else{
