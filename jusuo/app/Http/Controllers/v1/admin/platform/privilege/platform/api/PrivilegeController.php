...
namespace App\Http\Controllers\v1\admin\platform\privilege\platform\api;
...
class PrivilegeController extends ApiController
{
    ...
    public function index(Request $request)
    {
        ...
        $datas->transform(function($v){
            $v->shown_text = PrivilegePlatform::shownGroup($v->shown);
            $v->is_menu_text = PrivilegePlatform::isMenuGroup($v->is_menu);
    ...
    public function ajax_parent_privilege(Request $request)
    {
        ...
        $entry = PrivilegePlatform::select(['id','display_name','name','description'])
            ->orderBy('sort','asc')
            ->orderBy('created_at','desc');
            //->where('is_menu',PrivilegePlatform::IS_SHOW_MENU_YES) //默认只显示菜单权限
    ...
    public function store(Request $request)
    {
        ...
        $exist_name = PrivilegePlatform::OfName($input_data['name'])
            ->first();
        if($exist_name){$this->respFail('权限名称已存在！');}

        $parent_privilege = null;
        if($input_data['parent_id']){
            $parent_privilege = PrivilegePlatform::find($input_data['parent_id']);
            if(!$parent_privilege){
        ...
        try{
            ...
            //新建权限
            $privilege = new PrivilegePlatform();
    ...
    public function update(Request $request)
    {
        ...
        $privilege = PrivilegePlatform::find($input_data['id']);
        ...
        try{
            ...
            if($origin_parent_id != $new_parent_id){
                ...
                $child_privileges = PrivilegePlatform::where('path','like','%'.$privilege->id.'%')
                    ->orderBy('level','asc')
                    ->get();

                foreach($child_privileges as $child_privilege){
                    $parent_privilege = PrivilegePlatform::find($child_privilege->parent_id);
                    $path = [0];
                    $level = 0;
                    if($parent_privilege){
                        $path = explode(',',$parent_privilege->path);
            ...
            if($origin_is_super_admin == 0 && $new_is_super_admin == 1){
                ...
                $invalid_privileges = DB::table('role_privilege_platforms as rpo')
                    ->select(['rpo.id'])
                    ->join('role_platforms as ro','ro.id','=','rpo.role_id')
                ...
                if($invalid_privileges->count()>0){
                    ...
                    RolePrivilegePlatform::whereIn('id',$ids)->delete();
                }
            }
            ...
            DB::rollback();
            SystemLogService::simple('修改平台权限信息',array(json_encode($e)));
            $this->respFail('系统错误！');
    ...
    public function destroy(Request $request)
    {
        ...
        $data = PrivilegePlatform::find($input_data['id']);
