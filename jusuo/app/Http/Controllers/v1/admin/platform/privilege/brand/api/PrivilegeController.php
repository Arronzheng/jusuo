...
namespace App\Http\Controllers\v1\admin\platform\privilege\brand\api;
...
class PrivilegeController extends ApiController
{
    ...
    public function index(Request $request)
    {
        ...
        $datas->transform(function($v){
            ...
            $v->parent_name = '';
            $parent_privilege = PrivilegeBrand::find($parent_id);
            if($parent_privilege){
                $v->parent_name = $parent_privilege->display_name;
            }
            $v->shown_text = PrivilegeBrand::shownGroup($v->shown);
            $v->is_menu_text = PrivilegeBrand::isMenuGroup($v->is_menu);
    ...
    //更新权限排序
    public function nestable_sort(Request $request)
    {
        ...
        $flattened = $collection->flatten(10);
        ...
        DBService::updateBatch('privilege_brands',$updateData);
    ...
    public function ajax_parent_privilege(Request $request)
    {
        ...
        $entry = PrivilegeBrand::select(['id','path','sort','created_at','display_name','name','description'])
            ->orderBy('path','asc')
            ->orderBy('sort','asc')
    ...
    public function store(Request $request)
    {
        ...
        $exist_name = PrivilegeBrand::OfName($input_data['name'])
            ->first();
        if($exist_name){$this->respFail('权限名称已存在！');}

        $parent_privilege = null;
        if($input_data['parent_id']){
            $parent_privilege = PrivilegeBrand::find($input_data['parent_id']);
            if(!$parent_privilege){
        ...
        try{
            ...
            $privilege->guard_name = 'brand';
            $privilege->save();

            //更新超级管理员角色权限（20191125 超级管理员权限由上级决定）
            //PrivilegeService::sync_super_admin_role_privileges('brand');

            //如果增加某品牌权限，则要同步给赋予了本权限的一级父权限的品牌超级管理员(20191125)
            $path_array = explode(',',$path);
            if(isset($path_array[1])){
                $lv0_privilege_id = $path_array[1];
                $lv0_privilege = PrivilegeBrand::find($lv0_privilege_id);
                if($lv0_privilege){
    ...
    public function update(Request $request)
    {
        ...
        $privilege = PrivilegeBrand::find($input_data['id']);
        ...
        try{
            ...
            if($origin_parent_id != $new_parent_id){
                ...
                $child_privileges = PrivilegeBrand::where('path','like','%'.$privilege->id.'%')
                    ->orderBy('level','asc')
                    ->get();

                foreach($child_privileges as $child_privilege){
                    $parent_privilege = PrivilegeBrand::find($child_privilege->parent_id);
                    $path = [0];
                    $level = 0;
                    if($parent_privilege){
                        $path = explode(',',$parent_privilege->path);
            ...
            if($origin_is_super_admin == 0 && $new_is_super_admin == 1){
                ...
                $invalid_privileges = DB::table('role_privilege_brands as rpo')
                    ->select(['rpo.id'])
                    ->join('role_brands as ro','ro.id','=','rpo.role_id')
                ...
                if($invalid_privileges->count()>0){
                    ...
                    RolePrivilegeBrand::whereIn('id',$ids)->delete();
                }
            }
            ...
            DB::rollback();
            SystemLogService::simple('修改品牌权限信息',array(json_encode($e)));
            $this->respFail('系统错误！');
    ...
    public function destroy(Request $request)
    {
        ...
        $data = PrivilegeBrand::find($input_data['id']);
