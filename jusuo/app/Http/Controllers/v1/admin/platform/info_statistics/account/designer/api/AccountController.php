...
namespace App\Http\Controllers\v1\admin\platform\info_statistics\account\designer\api;
...
class AccountController extends ApiController
{
    ...
    public function account_index(Request $request)
    {
        ...
        $organization_type = $request->input('org',null);
        $organization_id = $request->input('orgid',null);

        $entry = DB::table('designers as dsn')->groupBy('dsn.id');

        //筛选组织
        if($organization_id!==null){
            $entry = $entry->where('dsn.organization_id',$organization_id);
        }
        if($organization_type!==null){
            if(in_array($organization_type,['0','1','2','3'])){
                switch($organization_type){
                    case '0':$organization_type = Designer::ORGANIZATION_TYPE_NONE;break;
                    case '1':$organization_type = Designer::ORGANIZATION_TYPE_BRAND;break;
                    case '2':$organization_type = Designer::ORGANIZATION_TYPE_SELLER;break;
                    case '3':$organization_type = Designer::ORGANIZATION_TYPE_DESIGNER_COMPANY;break;
                }

                $entry->where('dsn.organization_type',$organization_type);
            }
        }
        ...
        $entry->join('designer_details as detail','detail.designer_id','=','dsn.id')
            ...
            "))
            ->select([
                'dsn.id as designer_id','dsn.top_platform_status','detail.nickname','detail.realname','dsn.login_mobile',
        ...
        $datas->transform(function($v){
            ...
            $v->changeStatusApiUrl = url('admin/platform/info_statistics/account/designer/api/'.$v->designer_id.'/top_status');
            //所属组织
            $v->organization = '无';
    ...
    public function change_status($id, Request $request)
    {
        ...
        try{
            ...
            //更新状态
            if($designer->top_platform_status==Designer::TOP_PLATFORM_STATUS_OFF){
                $designer->top_platform_status = Designer::TOP_PLATFORM_STATUS_ON;
                $designer->top_platform_time = Carbon::now();
                ...
                $designer->top_platform_status = Designer::TOP_PLATFORM_STATUS_OFF;
            }
