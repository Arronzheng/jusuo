...
namespace App\Http\Controllers\v1\admin\platform\info_statistics\account\seller\api;
...
class AccountController extends ApiController
{
    ...
    public function account_index(Request $request)
    {
        ...
        $entry->join('organization_dealers as d','as.dealer_id','=','d.id')
            ...
            ->select([
                ...
                'd.status as seller_status','d.top_platform_status',
        ...
        $datas->transform(function($v){
            ...
            $v->changeStatusApiUrl = url('admin/platform/info_statistics/account/seller/api/'.$v->seller_id.'/top_status');
            $v->area_belong_text = '';
            if ($v->area_belong_id){
    ...
    public function change_status($id, Request $request)
    {
        ...
        try{
            ...
            //更新状态
            if($seller->top_platform_status==OrganizationDealer::TOP_PLATFORM_STATUS_OFF){
                $seller->top_platform_status = OrganizationDealer::TOP_PLATFORM_STATUS_ON;
                $seller->top_platform_time = Carbon::now();
            }else{
                $seller->top_platform_status = OrganizationDealer::TOP_PLATFORM_STATUS_OFF;
            }
