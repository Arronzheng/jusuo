...
namespace App\Http\Controllers\v1\admin\platform\album\index\hd\free_designer\api;
...
class AlbumController extends ApiController
{
    ...
    public function index(Request $request)
    {
        ...
        $entry = Album::query()
            ->select([
                ...
                'albums.id','type','title','code','status','designer_id','updated_at','count_area','visible_status'
            ])
            ->whereHas('designer',function($query){
                $query->where('organization_type','');
        ...
        $datas->transform(function($v){
            ...
            $v->changeStatusApiUrl = url('admin/platform/album/index/hd/free_designer/api/'.$v->id.'/status');
    ...
    public function change_status($id, Request $request)
    {
        ...
        try{
            ...
            //更新状态
            if($data->visible_status==Album::VISIBLE_STATUS_ON){
                //下架
                $data->visible_status = Album::VISIBLE_STATUS_OFF;
                ...
                //上架
                $data->visible_status = Album::VISIBLE_STATUS_ON;
