...
namespace App\Http\Controllers\v1\admin\seller\security_center\api;
...
class SecurityCenterController extends ApiController
{
    ...
    //提交密保问题
    public function secret_question(Request $request)
    {
        $input_data = $request->all();
        ...
        //字符数量
        if(iconv_strlen($input_data['secret_question'],"UTF-8")>50){
            $this->respFail('密保问题不能超过50个字符');
        }
        if(iconv_strlen($input_data['secret_answer'],"UTF-8")>50){
            $this->respFail('密保答案不能超过50个字符');
        }

        //保存
        $admin->secret_question = $input_data['secret_question'];
        $admin->secret_answer = $input_data['secret_answer'];
        $admin->save();
