...
namespace App\Http\Controllers\v1\admin\seller\info_statistics\album\api;
...
class AlbumController extends ApiController
{
    ...
    public function index(Request $request)
    {
        ...
        $entry = Album::query()
            ->select([
                ...
                'albums.top_status_dealer','albums.designer_id','albums.period_status'
            ])
            ->with(['designer:id','designer.detail:id,designer_id,realname'])
            ->whereHas('designer',function($designer)use($seller_id){
                $designer->where('organization_type',Designer::ORGANIZATION_TYPE_SELLER);
                $designer->whereHas('seller',function($organization)use($seller_id){
                    $organization->where('organization_dealers.id',$seller_id);
                });
            });
        ...
        $datas->transform(function($v){
            ...
            $v->changeStatusApiUrl = url('admin/seller/info_statistics/album/api/'.$v->id.'/top_status');
    ...
    public function change_status($id, Request $request)
    {
        ...
        try{
            ...
            //更新状态
            if($album->top_status_dealer==Album::TOP_PLATFORM_OFF){
                $album->top_status_dealer = Album::TOP_PLATFORM_ON;
            }else{
                $album->top_status_dealer = Album::TOP_PLATFORM_OFF;
    ...
    public function get_sellers(Request $request)
    {
        ...
        $entry = OrganizationBrand::select(['id','name','seller_name','organization_account','contact_name','contact_telephone'])
            ->orderBy('created_at','desc');

        if($keyword){
            $entry->where(function($query) use($keyword){
                $query->where('name','like',"%".$keyword."%");
                $query->orWhere('seller_name','like',"%".$keyword."%");
