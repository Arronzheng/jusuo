...
class AlbumNotifyController extends ApiController
{
    ...
    public function index(Request $request)
    {
        ...
        $datas->transform(function($v) use($loginAdmin){
            $v->type_text = MsgAlbumDealer::typeGroup($v->type);
            $v->is_read_text = MsgAlbumDealer::isReadGroup($v->is_read);
            $v->setReadApiUrl = url('admin/seller/info_notify/api/album_notify/'.$v->id.'/set_read');
            return $v;
        });
    ...
    public function set_read($id, Request $request)
    {
        ...
        try{
            ...
            $msg = $entry->find($id);
            $msg->is_read = MsgAlbumDealer::IS_READ_YES;
            $msg->save();
